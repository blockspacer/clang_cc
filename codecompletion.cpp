...
#ifdef CLANGCC_TIMING
    LoggerAccess::Get()->Log(wxString::Format("ProcessCodeCompleteResults Sort by Priority executed in %ldms", watch.Time()),Logger::info);
#endif // CLANGCC_TIMING
...
#ifdef CLANGCC_TIMING
    LoggerAccess::Get()->Log(wxString::Format("ProcessCodeCompleteResults processed %d results in %ldms", NumResults, watch.Time()),Logger::info);
#endif // CLANGCC_TIMING
}
...
{
    LoggerAccess::Get()->Log("Strange!! ProcessOverloadCandidates never gets called.See to it.");
