...
int ClangCC::CodeComplete()
{
    ...
    wxString logstring;
    logstring << "Code Complete at : ") << editor->GetShortName() << _(":") << line << _(":" << column;
...
void ClangCC::OnEditorEvent(cbEditor* editor, wxScintillaEvent& sciEvent)
{
    ...
    if (evtype == wxEVT_SCI_CHARADDED) {
        LoggerAccess::Get()->Log("wxEvt_SCI_CHARAdded ");
        ...
        if (control->IsString(style) || control->IsComment(style) || control->IsCharacter(style)) {
            sciEvent.Skip();
            return;
        }
...
void ClangCC::OnEditorActivatedTimer(wxTimerEvent& event)
{
    ...
    LoggerAccess::Get()->Log("Editor activated : " + editor->GetFilename());
}
